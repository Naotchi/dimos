...
class BasicVoiceAssistant:
    ...
    async def _setup_session(self):
        ...
        if self.voice.startswith("en-US-") or self.voice.startswith("en-CA-") or "-" in self.voice:
            # Azure voice — apply speaking rate
            voice_config = AzureStandardVoice(name=self.voice, rate=self.voice_rate)
        else:
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.environ.get("AZURE_VOICELIVE_VOICE", "ja-JP-MayuNeural"),
    )

    parser.add_argument(
        "--voice-rate",
        help="Speaking rate for Azure voices (e.g. '1.2', '0.9', '+20%%').",
        type=str,
        default=os.environ.get("AZURE_VOICELIVE_VOICE_RATE", "1.2"),
    )
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            ...
            "ユーザーが何語で話しても、必ず日本語のみで自然かつ簡潔に応答してください。"
            "英語やその他の言語は使わないでください。",
        ),
    )
