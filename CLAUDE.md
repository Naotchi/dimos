...
**fork の差分は「新規ファイル追加」を基本とし、upstream にあるファイルの編集は最小限に抑える。** 目的は upstream/main を ff-only に近い形で取り込み続けられる状態を保つこと。

- fork 固有機能は **新規ファイルで追加**する（例: `*_ja.py`、`scripts/bench_*.py` のように別ファイル化）。
- upstream 既存ファイルへの編集は、**新規ファイルを登録するための最小差分**（blueprint 登録など）に限る。ロジック改変や clean-up を upstream ファイルに加えない。
- 既存挙動を変えたい場合も、まず「派生ファイルを新規追加 → 呼び出し側で切り替え」を検討する。
