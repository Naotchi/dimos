...
"""``unitree-go2-agentic-local-tts`` + shelf-row detection (fork-local).
...
The only difference from :mod:`unitree_go2_agentic_local_tts_detection` is
that ``Detection3DModule`` is configured with ``detector=ShelfRowDetector``
(fork-local shelf-row crop detector) instead of the default COCO detector.
