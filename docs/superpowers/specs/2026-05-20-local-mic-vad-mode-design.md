...
- マイク既定: 16kHz mono int16、`block_size=1024`（=64ms）。`block_size` は
  PortAudio のソフトウェアバッファ設定でハードウェア非依存。vad モードでは
  silero v5 の 512 サンプル(32ms)窓に合わせて `block_size=512` で開く。
...
- **vad モードではマイクの `block_size` を silero のチャンク幅（16kHz=512 /
  8kHz=256）に上書きして開く**。これにより 1 フレーム = 1 VAD 窓となり、segmenter
  側の再チャンク（端数キャリー）が不要になる。hold モードは従来の `block_size`
  （既定 1024）のまま。`block_size` は PortAudio のソフトウェアバッファ設定であり
  マイクのハードウェア依存ではないため、モードごとの上書きは安全。
...
- `chunk_samples_for(sample_rate)`: silero のチャンク幅を返す（16kHz=512 /
  8kHz=256）。`LocalMicrophoneJa` がマイクの `block_size` 決定に使う。
- `feed(frame: AudioEvent) -> AudioEvent | None`: **1 フレーム = 1 チャンク前提**
  （マイクを silero チャンク幅で開くため、再チャンク不要）。
  1. フレーム長がチャンク幅と異なれば `ValueError`（block_size 設定ミスを早期に検出）。
  2. idle 中は直近 `vad_speech_pad_ms` 分をプリロール用リングバッファに保持。
  3. チャンクを `VADIterator` に food。返り値:
...
- `start()`:
  - `mic_mode=="vad"`: `VadStreamSegmenter` を構築（silero ロード）。マイクを
    `segmenter.chunk`（silero チャンク幅）の `block_size` で開く。`mic_gate` は
    購読しない。
  - `mic_mode=="hold"`: マイクを `cfg.block_size`（既定 1024）で開く。現状通り
    `mic_gate` を購読、VAD は構築しない（silero も import しない）。
- `_on_audio(event)`:
...
2. **フレーム検証**: チャンク幅ちょうどのフレームはそのまま VADIterator に渡る /
   異なる長さのフレームは `ValueError`。`chunk_samples_for` が 16k→512 / 8k→256 を返す。
