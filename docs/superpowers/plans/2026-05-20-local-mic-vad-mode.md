...
- **Create** `dimos/agents/vad_segmenter_ja.py` — `VadStreamSegmenter`。再チャンク・プリロール・最小発話長・最大長打ち切りを内包。silero に依存するのは `from_config` のみ。
...
## Task 2: VadStreamSegmenter の再チャンク（feed のキャリーバッファ）

**Files:**
- Create: `dimos/agents/vad_segmenter_ja.py`
- Test: `tests/agents/test_vad_segmenter_ja.py`

64ms(1024サンプル)フレームを silero の 512 サンプル単位へ割り、端数を次フレームへ持ち越す。本タスクでは VADIterator が常に `None`（無音）を返す前提で、チャンク分割と消費サンプル数だけを検証する。
...
def test_feed_rechunks_to_512_and_carries_remainder():
    it = FakeVadIterator()  # 常に None
    seg = VadStreamSegmenter(it, sample_rate=16000)
    # 1024 サンプル = 512 ×2 → 2 回 __call__、端数 0
    assert seg.feed(_event(1024)) is None
    assert it.calls == 2
    # 700 サンプル → 512 ×1（端数 188 をキャリー）→ 累計 3 回
    assert seg.feed(_event(700)) is None
    assert it.calls == 3
    # さらに 324 サンプル投入で 188+324=512 → もう 1 回（累計 4 回）
    assert seg.feed(_event(324)) is None
    assert it.calls == 4
```
...
Wraps silero ``VADIterator``. Audio frames (any block size) are re-chunked
into the fixed window silero expects (512 samples @ 16 kHz / 256 @ 8 kHz),
fed one chunk at a time, and assembled into a single utterance ``AudioEvent``
on the falling edge of speech. Used by :class:`LocalMicrophoneJa` in vad mode.
...
class VadStreamSegmenter:
    ...
    def __init__(
        ...
        *,
        ...
        self._chunk = _CHUNK_FOR_RATE[sample_rate]
    ...
    def feed(self, event: AudioEvent) -> AudioEvent | None:
        """Consume one mic frame; return a finished utterance or None."""
        data = np.asarray(event.data, dtype=np.int16).reshape(-1)
        buf = np.concatenate([self._carry, data]) if self._carry.size else data
        n_full = (buf.size // self._chunk) * self._chunk
        self._carry = buf[n_full:].copy()
        result: AudioEvent | None = None
        for start in range(0, n_full, self._chunk):
            out = self._process_chunk(buf[start : start + self._chunk], event)
            if out is not None:
                result = out
        return result
...
Run: `python -m pytest tests/agents/test_vad_segmenter_ja.py -v`
Expected: PASS（1 件、`it.calls` の回数が一致）

- [ ] **Step 5: コミット**

```bash
git add dimos/agents/vad_segmenter_ja.py tests/agents/test_vad_segmenter_ja.py
git commit -m "feat(vad): VadStreamSegmenter re-chunking skeleton"
```
...
`tests/agents/test_vad_segmenter_ja.py` に追記。512 サンプルちょうどのイベントを使い 1 feed = 1 チャンクに揃える:
...
Run: `python -m pytest tests/agents/test_vad_segmenter_ja.py -v`
Expected: PASS（再チャンク 1 + 本タスク 4 = 5 件）
...
```python
        ...
        if cfg.mic_mode == "vad":
            ...
            logger.info(
                "LocalMicrophoneJa started in VAD mode (device=%s, sr=%d Hz, block=%d)",
                cfg.device_index,
                cfg.sample_rate,
                cfg.block_size,
            )
            ...
            self._gate_unsub = self.mic_gate.subscribe(self._on_gate)
            logger.info(
                "LocalMicrophoneJa started in hold/PTT mode (device=%s, sr=%d Hz, block=%d)",
                cfg.device_index,
                cfg.sample_rate,
                cfg.block_size,
            )
...
- **既知の制約**: `feed` が 1 フレーム内で複数 `end` を受けた場合は最後の発話のみ返す（64ms フレームでは実害なし、スコープ外に記載済みの単純化）。
