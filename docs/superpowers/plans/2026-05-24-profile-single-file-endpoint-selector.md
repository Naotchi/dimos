...
### Task 2: ~~Create root `.env.example`~~ — DROPPED

The harness denies writes to `.env*` (secret-protection guard), which also blocks a committed `.env.example` template. Per user decision (2026-05-24), the endpoint-key template is documented in `docs/env-vs-config.md` instead (folded into Task 6). No file is created here.
