...
### 追試(2026-08-24): router の peer 中継は設定でも有効化できない

zenoh 1.9 では旧 `peers_failover_brokering` 設定が廃止されている(DEFAULT_CONFIG.json5
に存在せず、`gateway.south` / region 概念に置換)。分離ネットワーク + 二重ホーム router
のマトリクス試験の結果:
...
- multicast: WAN 越え不可
- gossip → ピア直結: gossip が広告するのは各セッションのローカル IP
  (VM は 10.x、物理は 192.168.x)なので相互にダイヤル不能 → 不成立
- router 中継: 上記のとおり peer モードでは不成立

**したがって、コード変更なしで動く構成は「オーバーレイネットワークで相互 TCP
到達性を作る」一択**(Tailscale / WireGuard)。オーバーレイの仮想 IP は実インター
フェースとして gossip に広告されるため、コンテナ検証で PASS 済みの
「router = gossip ハブ + ピア直結」構成に帰着する(zenohd は VM 上でよい)。

コード変更を許すなら代替が 2 つある(いずれも upstream 提案候補):

1. `ZenohConfig.listen` を env(例 `ZENOH_LISTEN`)から設定可能にする —
   VM 側の各セッションが既知ポートで listen し、NAT 裏側から `ROBOT_IPS` で
   全ポートに直接ダイヤルする(片方向到達性だけで成立)
2. client モードを env から選択可能にする — VM 上の zenohd が中継する
   (client は router 必須になるトレードオフ)

### 展示(MEC オフロードデモ)観点の注意

本検証の最終目的は「Edge 処理のクラウドオフロード」展示(本番: docomo MEC /
Compute E、低遅延が売り)。この文脈では:

- **Tailscale は展示非推奨**: NAT 越え失敗時に DERP 中継へ黙ってフォールバックし、
  インターネット経由になって MEC の低遅延ストーリーが実際に崩れる。切り替わりは
  外から見えにくく、展示中の原因究明が困難。
- WireGuard 直結(静的 2 点間)なら追加遅延は 1ms 未満で経路も決定的。
  オーバーヘッドは LAN 上で有無比較のベンチを取り定量提示可能。
- **本命は MEC 閉域網のネイティブ相互到達性**(専用 APN / 閉域接続)。相互到達なら
  NAT 問題自体が消え、オーバーレイなしの「素の TCP 直結」で展示できる。
  要確認事項: 「MEC 側からエッジ側デバイスの IP に TCP 接続できるか」。
  片方向のみなら上記 1(`ZENOH_LISTEN`)が最もストーリーの濁らない選択肢。
